...
void set_velocity(coordinate dest, ros::Publisher cmd_vel_pub, double velocity) {
	...
	ROS_INFO("\n\nSetting velocity as %lf, %lf at angle %lf.\n\n", x_diff / mag, y_diff / mag, fmod(atan2(y_diff, x_diff)*180/M_PI + 360, 360));
	cmd_vel_msg.header.stamp = ros::Time::now();
...
int main(int argc, char** argv)
{
	...
	while(ros::ok())
	{
		...
		if(check_if_waypoint_reached(target, 5)){
		...
		if (object_ahead && tangent_going) {
			if (check_if_waypoint_reached(next_coordinate, 5)){ // prev = 3
				...
				set_velocity(next_coordinate, cmd_vel_pub, 1);
			}
		}
		if (! tangent_going && object_ahead){
			next_coordinate = get_next_coordinate_from_tangents();
			stak.push(next_coordinate);
			set_velocity(next_coordinate, cmd_vel_pub, 1);   // set_destination(next_coordinate.x, next_coordinate.y, 1.5, 0);
