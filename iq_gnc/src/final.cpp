...
float get_angle (double x, double y) {
	...
	float angle = (float) angle_rad * (180 / M_PI);
	if (angle < 0) {
		angle += 360;
	}
	return angle;
...
void set_velocity(coordinate dest, ros::Publisher cmd_vel_pub, double velocity) {
	...
	cur_angle = get_angle(x_diff, y_diff);
	ROS_INFO("\nDrone is going to angle | %f degrees.\n", cur_angle);
	cmd_vel_msg.header.stamp = ros::Time::now();
...
void laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg){
	...
	target_angle = fmod((target_angle + yawd + 360), 360);   // target_angle = fmod((target_angle + 90 - heading + 360), 360);
...
/*
	if (x_diff > 0 && y_diff > 0)
		...
		target_angle = fmod((-target_angle + 270), 360);
*/
	ROS_INFO("X_Diff ---> %lf, Y_Diff ---> %lf, target_angle ---> %lf", x_diff, y_diff, target_angle);
	...
	for(int i=0; i<720; i++){
		...
		if (prev_dist > max_range && dist < max_range || prev_dist < max_range && dist > max_range){
			...
			if (prev_dist > max_range)
				...
				current_angle += 30;
			possible_coordinate.x = current_pos.x + ( std::min(dist, prev_dist) - 2 ) * cos(current_angle);
			possible_coordinate.y = current_pos.y + ( std::min(dist, prev_dist) - 2 ) * sin(current_angle);
...
coordinate get_next_coordinate_from_tangents() {
	...
	for (int i=1; i<tangent_points.size(); i++) {
		...
		if (cost + heuristic > new_cost + new_heuristic) {
			small = i;
			cost = new_cost, heuristic = new_heuristic;
		}
		insert_in_seen_coordinates(tangent_points[i]);
	}
